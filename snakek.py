...
class Food: 
    def __init__(self):
        while True:
            x = random.randint(2, (WINDOW_WIDTH//SPACE_SIZE - 3)) * SPACE_SIZE
            y = random.randint(2, (WINDOW_HEIGHT//SPACE_SIZE - 3)) * SPACE_SIZE
...
window = Tk()
window.title('Змейка')
window.resizable(False, False)  # запрещаем растяжение окна

score = 0
direction = 'down'

label_score = Label(window, text='Очки: {}'.format(score), font=('Arial', 40))
label_score.pack()
...
window.geometry('600x500')  # разрешение окна
